(() => {
  ...
  var collapsibleWhitespaceRunRe, needsWhitespaceNormalizationRe, sharedWordSegmenter, segmenterLocale, arabicScriptRe, combiningMarkRe, decimalDigitRe, keepAllGlueChars, keepAllDashBreakChars, kinsokuStart, kinsokuEnd, forwardStickyGlue, leftStickyPunctuation, arabicNoSpaceTrailingPunctuation, myanmarMedialGlue, closingQuoteChars, lineBreakNumericAffixRanges, breakCharRe, urlSchemeSegmentRe, numericJoinerChars, noSpacePunctuationChainJoiners;
  var init_analysis = __esm({
    "node_modules/@chenglou/pretext/dist/analysis.js"() {
      ...
      keepAllDashBreakChars = /* @__PURE__ */ new Set([
        "-",
        "\u2010",
        "\u2013",
        "\u2014"
      ...
      noSpacePunctuationChainJoiners = /* @__PURE__ */ new Set([".", ",", ":", ";"]);
    }
  });
  ...
  // web-src/pretext_breaker.js
  var require_pretext_breaker = __commonJS({
    "web-src/pretext_breaker.js"() {
      init_layout();
      ...
      var PROSE = "PRETEXT  BREAKER  turns  text  into  terrain.  Every  frame,  the  prose  field  is  measured,  split,  and  reflowed  around  moving  obstacles.  The  ball  cuts  a  path  through  language.  The  paddle  bends  paragraphs.  Bricks  are  not  sprites;  they  are  words  waiting  to  be  broken.  This  is  the  trick:  no  boring  rectangle  of  text,  no  static  wallpaper,  just  layout  reacting  like  a  game  system.  ";
      ...
      function drawReactiveProse() {
        ...
        for (var y = 52; y < proseBottom; y += lineHeight) {
          ...
          if (state2.active || Math.abs(state2.gapX1 - state2.gapX2) > 1) {
            ...
            ctx.textAlign = "right";
            ctx.fillText(leftPart, state2.gapX1, y);
            ctx.textAlign = "left";
            ctx.fillText(rightPart, state2.gapX2, y);
          } else {
            ctx.textAlign = "left";
      ...
      function burst(x, y, color, count) {
        for (var i = 0; i < count; i++) {
          particles.push({ x, y, vx: rand(-2.3, 2.3), vy: rand(-2.5, 1.3), life: rand(0.45, 1), color, text: WORDS[Math.floor(rand(0, WORDS.length))].charAt(0) });
        }
      }
      function draw() {
        ctx.save();
